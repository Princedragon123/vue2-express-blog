...
const BlogController = {
    ...
    //删除已上传但未发布的媒体文件
    removeUploadedFile: async (req, res) => {
        try {
            const { fileUrl } = req.body
            if (!fileUrl) {
                return res.status(400).json({ success: false, message: '缺少文件Url' })
            }
            const deleted = uploadService.deleteFile(fileUrl)
            if (deleted) {
                res.json({ success: true, message: '文件已删除' })
            } else {
                res.json({ success: true, message: '文件不存在或者已被删除' })
            }
        } catch (error) {
