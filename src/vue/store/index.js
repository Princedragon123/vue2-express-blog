...
// │       │               │               │               │               │
// │       │               │               │               │               │
// │  存储原始数据     从state派生数据   唯一修改state    处理异步后commit   │
// │  类似data        类似computed      的方式           类似methods        │
// │                                                                         │
// └─────────────────────────────────────────────────────────────────────────┘
//
// 【数据流向 - 单向数据流】
...
// │       │ state.token = token                                            │
// │       ▼                                                                 │
// │   State 变化触发 Getter 更新                                           │
// │   ──────────────────────────                                           │
// │       │                                                                 │
// │       │ getters.isLoggedIn 返回新值                                    │
// │       ▼                                                                 │
// │   组件自动更新（响应式）                                                │
// │                                                                         │
// └─────────────────────────────────────────────────────────────────────────┘
...
// │  ──────────────────────────                                             │
// │  const auth = {                                                         │
// │    state: Vue.observable({ user: null, token: null }),                 │
// │    loginSuccess(user, token) {                                          │
// │      Vue.set(this.state, 'user', user);   // 手动 Vue.set             │
// │      this.state.token = token;            // 直接改，无法追踪          │
// │    }                                                                    │
// │  };                                                                     │
// │  // 使用：auth.state.user / auth.loginSuccess()                        │
// │                                                                         │
// │  ✅ 新方案（Vuex）                                                      │
// │  ──────────────                                                         │
...
// │    mutations: {                                                         │
// │      SET_USER(state, user) { state.user = user; }  // 不需要 Vue.set  │
// │    },                                                                   │
// │    actions: {                                                           │
// │      loginSuccess({ commit }, payload) {                                │
// │        commit('SET_USER', payload.user);  // 必须走 commit，可追踪    │
// │      }                                                                  │
// │    }                                                                    │
// │  });                                                                    │
// │  // 使用：this.$store.state.user / this.$store.dispatch('loginSuccess')│
// │                                                                         │
// │  【Vuex 优势】                                                          │
// │  1. 修改可追踪 - DevTools 能看到每次 state 变化                        │
// │  2. 强制规范 - 必须通过 commit 修改，不能直接改 state                  │
// │  3. 计算缓存 - getters 有缓存，依赖不变就不重算                        │
// │  4. 异步分离 - mutations 只同步，actions 处理异步                      │
// │  5. 不需要 Vue.set - Vuex 内部自动处理响应式                           │
// │                                                                         │
// └─────────────────────────────────────────────────────────────────────────┘
//
// 【面试常问】
...
//
// 【读取优先级】localStorage > sessionStorage
...
export default new Vuex.Store({
  ...
  // 【state - 状态/数据】
  // ========================================================
  // 【概念】类似组件的 data，存储应用的全局数据
  // 【规则】只能通过 mutations 修改，不能直接改
  // 【访问】this.$store.state.token（但不推荐，推荐用 getters）
  //
  // 【和 Vue.observable 的对比】
  ...
  // 【getters - 计算属性】
  // ========================================================
  // 【概念】类似组件的 computed，从 state 派生数据
  // 【优势】有缓存！依赖不变就不会重新计算
  // 【访问】this.$store.getters.isLoggedIn
  //
  // 【和 Vue.observable 的对比】
  ...
  getters: {
    ...
    // 【原理】JWT Token 的第二段是 Base64 编码的 Payload
    // 【步骤】
  ...
  // 【mutations - 同步修改】
  // ========================================================
  // 【概念】唯一修改 state 的方式，必须是同步函数
  // 【规则】
  //   1. 函数名全大写 + 下划线（约定俗成）
  //   2. 第一个参数是 state，第二个是传入的值
  //   3. 只能做同步操作，不能写 setTimeout、fetch 等
  // 【调用】this.$store.commit('SET_TOKEN', 'xxx')
  ...
  actions: {
    ...
    loginSuccess({ commit }, { user, token, rememberMe }) {
      ...
      // 2. 持久化到浏览器存储（刷新页面后还能恢复）
      // rememberMe = true  → localStorage（关闭浏览器还在）
      // rememberMe = false → sessionStorage（关闭浏览器就没了）
